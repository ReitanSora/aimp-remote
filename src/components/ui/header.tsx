...
export default function Header({ hasSearchBar = true, searchBarVisible, searchValue, setSearchBarVisible, setSearchValue, leftSideOnPress, LeftSideIconSet, iconName, iconSize, iconColor, title }: SearchBarProps) {
  ...
  return (
    <View style={styles.header}>
      {hasSearchBar ? (
        <>
          <Animated.View pointerEvents={searchBarVisible ? 'none' : 'auto'} style={[styles.normalWrapper, animatedNormalHeader]}>
            <NormalButton rippleColor="rgba(139, 139, 139, 0.5)" onPress={() => leftSideOnPress()} IconSet={LeftSideIconSet} iconName={iconName} iconSize={iconSize} iconColor={iconColor} />
            <NormalButton rippleColor="rgba(139, 139, 139, 0.5)" onPress={() => handleShowSearchbar()} IconSet={Ionicons} iconName="search" />
          </Animated.View>
          <Animated.View pointerEvents={searchBarVisible ? 'auto' : 'none'} style={[styles.searchbarWrapper, animatedSearchbar]}>
            <View
              style={{
                width: 48,
                height: 48,
            ...
            />
            <NormalButton rippleColor="transparent" onPress={() => handleCloseSearchbar()} IconSet={Ionicons} iconName="close" />
          </Animated.View>
        </>
      ) : (
