...
export default function SongDetailsModal() {
  ...
  return (
    <Animated.View style={styles.extraInfo} entering={FadeIn}>
          ...
          >
            <View style={{ flexDirection: 'row' }}>
              {[...Array(songInfo.rating)].map((_, index) => (
                <MaterialCommunityIcons name="music-note-quarter" size={24} color="white" key={`rating-music-note-${index}`} />
              ))}
              {[...Array(5 - songInfo.rating)].map((_, index) => (
                <MaterialCommunityIcons name="music-note-half" size={24} color="#8B8B8B" key={`rating-music-note-${index}`} />
              ))}
            </View>
          </View>
        </View>
